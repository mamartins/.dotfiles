...
#ln -s all dotfiles from dropbox
read -p "Creating symlinks from ~/Dropbox/Dotfiles to ~. This may overwrite existing files in your home directory. Are you sure? (y/n) " -n 1
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
...
echo "checking brew"
command -v brew >/dev/null 2>&1 && echo "Brew already installed" || { echo >&2 "Brew not installed, installing"; ruby -e "$(curl -fsSL https://raw.github.com/mxcl/homebrew/go)"; }
